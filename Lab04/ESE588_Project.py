...
# np.savetxt("MVNDistro.csv", x_train, delimiter=",")

# import GMMData.csv
x_train_pandas = pd.read_csv('GMMData.csv')
x_train = x_train_pandas.to_numpy()

numberOfComponents = 6 # change to 6 for part 2 of project

# train GMM with EM
gmm = GaussianMixture(n_components=numberOfComponents, covariance_type='full').fit(x_train)

# clustering with K-means
k_means = KMeans(n_clusters=numberOfComponents, random_state=0).fit(x_train)
